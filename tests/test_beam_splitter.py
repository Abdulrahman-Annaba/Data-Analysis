...
def test_beam_splitter_initialization():
    ...
    computation_csv_path = Path(
        "../Trials/GH13-12V (DOWN) (5)/computation_parameters.csv")
    computation_parameters = pd.read_csv(computation_csv_path, index_col=0)
    optical_coefficients = computation_parameters.loc[[
        "A", "B"]][["RV", "RH", "TV", "TH"]]
    ...
    beam_splitter = BeamSplitter(
        optical_coefficients, transmitted_power_label, reflected_power_label)
    ...
    optical_coefficients = optical_coefficients.loc[[
        "A", "B"]][["RV", "RH", "TV"]]
...
def test_incident_power():
    ...
    computation_parameters = pd.read_csv(
        trial_path / "computation_parameters.csv", index_col=0)
    optical_coefficients = computation_parameters.loc[[
        "A", "B"]][["RV", "RH", "TV", "TH"]]
    # Get data at incident angle = 18
    data = data[data[:, 3] == -18]
    transmitted_power = data[:, 2]
    reflected_power = data[:, 1]
    # Compute correct incident power directly
    reflected_power_background = float(
        computation_parameters.loc["A"]["Background Power (W)"])
    reflectivity = float(optical_coefficients.loc["A"]["RH"])
    correct_incident_power = (
        reflected_power - reflected_power_background)/reflectivity
    transmitted_power_label = PowerMeterLabel.B
    reflected_power_label = PowerMeterLabel.A
    # Make BeamSplitter
    beam_splitter = BeamSplitter(
    ...
    assert np.array_equal(incident_power, correct_incident_power)
...
def test_efficiency():
    ...
    transmitted_power = data[:, 2]
    reflected_power = data[:, 1]
    # Compute correct efficiency directly
    reflected_power_background = float(
        computation_parameters.loc["A"]["Background Power (W)"])
    transmitted_power_background = float(
        computation_parameters.loc["B"]["Background Power (W)"])
    reflectivity = float(optical_coefficients.loc["A"]["RH"])
    correct_efficiency = (transmitted_power - transmitted_power_background) / \
        ((reflected_power - reflected_power_background)/reflectivity)
    ...
    print("correct", correct_efficiency)
    print("test", efficiency)
    assert np.array_equal(efficiency, correct_efficiency)
